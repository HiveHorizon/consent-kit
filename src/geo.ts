 ...
 */
export function cloudflareTrace(timeoutMs = 1500): () => Promise<string | null> {
  return async () => {
    try {
...
export const noGeo = (): Promise<string | null> => Promise.resolve(null);

/** Fixed country, for tests and local development. */
export function staticGeo(country: string | null): () => Promise<string | null> {
