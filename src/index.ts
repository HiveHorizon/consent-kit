 ...
 */
async function auditLegalPages(config: ConsentConfig): Promise<void> {
  const warn = (msg: string) => console.warn(`[consent-kit] ${msg}`);

  const privacyUrl = config.ui === false ? undefined : config.ui?.privacyUrl;
  if (!privacyUrl) {
    warn(
      "no ui.privacyUrl set, so the banner shows no policy link. Publish a " +
  ...
  if (
    ...
    warn(
      "no element on this page reopens the preferences. Consent must stay " +
...
async function expectPage(
  ...
  try {
    ...
    if (!res.ok) warn(`${label} points at ${url}, which returns ${res.status}.`);
  } catch {
...
export function initConsent(config: ConsentConfig): ConsentApi {
  ...
  if (debug) {
    (window as unknown as Record<string, unknown>).__consent = api;
